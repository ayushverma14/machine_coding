        ...
        int count=0;
            ...
            for(int j=0;j<pl.arr.size();j+=1)
            {
                for(int k=0;k<pl.arr.get(j).arr.size();k++)
                {
                    ...
                    if((f1=pl.arr.get(j).arr.get(k).type.equalsIgnoreCase(type.trim()))&&(f2=(pl.arr.get(j).arr.get(k).occupied==0)))
                    {
                     count++;
                    }
...
pl=new ParkingLot(op[1],Integer.parseInt(op[2]),Integer.parseInt(op[3]));
            ...
            pl.unparkingvehicle(op[1],op[2],op[3]);
           }
           else if(op.length==3)
           {
...
System.out.println("Lot Created");
    ...
    public void unparkingvehicle(String type,String mod,String color)
    {
        int f=0;
        for(int i=0;i<this.arr.size();i++)
        {
            for(int j=0;j<this.arr.get(i).arr.size();j++)
            {
                if(this.arr.get(i).arr.get(j).type.equals(type.trim()) && this.arr.get(i).arr.get(j).occupied==1 && this.arr.get(i).arr.get(j).model.equalsIgnoreCase(mod) && this.arr.get(i).arr.get(j).color.equals(color))
                {
